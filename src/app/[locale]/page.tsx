  ...
  return (
    <main className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center animate-in fade-in slide-in-from-bottom-10 duration-1000">
          <div className="inline-flex items-center gap-3 mb-8">
            ...
            <span className="text-amber-400/80 text-xs tracking-[0.3em] uppercase">{t.hero.location}</span>
            <span className="w-12 h-[1px] bg-amber-400/50"></span>
          </div>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3">
          <span className="text-white/30 text-xs tracking-widest uppercase rotate-90 origin-center translate-y-8">{t.hero.scroll}</span>
      ...
      <section className="py-32 bg-black relative">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 mb-20">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div>
              <span className="text-amber-400/80 text-xs tracking-[0.3em] uppercase block mb-4">{t.home.curatedForYou}</span>
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-light">
                {t.home.signatureExperiences.split(' ')[0]} <span className="font-serif italic text-amber-400">{t.home.signatureExperiences.split(' ').slice(1).join(' ')}</span>
              </h2>
            </div>
            <Link 
              href={`/${locale}/tours`}
      ...
      <section className="relative py-40 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          ...
          <div className="mt-12 flex items-center justify-center gap-4">
            ...
            <span className="text-white/40 text-sm tracking-widest uppercase">{t.home.quoteAuthor}</span>
            <span className="w-12 h-[1px] bg-amber-400/30"></span>
          </div>
        </div>
      </section>
      ...
      <section className="py-32 bg-zinc-950">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-20">
            <span className="text-amber-400/80 text-xs tracking-[0.3em] uppercase block mb-4">{t.home.difference}</span>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-light">
              {t.home.whyChooseUs.split(' ')[0]} Families Choose <span className="font-serif italic text-amber-400">{t.home.whyChooseUs.split(' ').slice(-2).join(' ')}</span>
            </h2>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-px bg-white/5">
            {t.home.values.map((value, idx) => (
              <div key={idx} className="bg-zinc-950 p-10 lg:p-12 group hover:bg-zinc-900 transition-colors duration-500">
                <span className="text-amber-400/20 text-6xl font-light block mb-6">0{idx + 1}</span>
      ...
      <section className="py-32 bg-black relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div>
              <span className="text-amber-400/80 text-xs tracking-[0.3em] uppercase block mb-8">{t.home.guestStories}</span>
                ...
                {testimonials.map((_, idx) => (
                  <button
                    ...
                    className={`h-[2px] transition-all duration-500 ${
                      currentTestimonial === idx ? 'w-12 bg-amber-400' : 'w-6 bg-white/20'
                    }`}
                  />
                ))}
              </div>
      ...
      <section className="py-32 bg-gradient-to-b from-zinc-950 to-black relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          <span className="text-amber-400/80 text-xs tracking-[0.3em] uppercase block mb-6">{t.home.beginJourney}</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-light mb-6">
            {t.home.ctaTitle.split(' ')[0]} to Create <span className="font-serif italic text-amber-400">{t.home.ctaTitle.split(' ').slice(1).join(' ')}</span>?
          </h2>
