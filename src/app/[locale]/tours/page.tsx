    ...
    <main className="min-h-screen bg-black text-white">
      ...
      <section className="relative h-[60vh] min-h-[500px] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          <div className="inline-flex items-center gap-3 mb-6">
            ...
            <span className="text-amber-400/80 text-xs tracking-[0.3em] uppercase">{t.tours.pageTitle}</span>
            <span className="w-8 h-[1px] bg-amber-400/50"></span>
          </div>
      ...
      <div className="bg-gradient-to-r from-emerald-900/30 via-emerald-800/20 to-emerald-900/30 border-y border-emerald-500/20 py-5">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex flex-col sm:flex-row items-center justify-center gap-3 text-center sm:text-left">
          <span className="text-emerald-400 font-medium">{t.tours.transportIncluded}</span>
          <span className="hidden sm:inline text-emerald-400/40">|</span>
          <span className="text-emerald-400/60 text-sm">{t.tours.transportDetail}</span>
        </div>
      </div>
            ...
            {filteredPackages.map((pkg) => (
              ...
              >
                <div className={`relative group ${pkg.id % 2 === 0 ? 'lg:order-2' : ''}`}>
                  ...
                  <div className={`absolute -bottom-4 -${pkg.id % 2 === 0 ? 'left' : 'right'}-4 w-full h-full border border-amber-400/10 -z-10 hidden lg:block`}></div>
                </div>
                ...
                <div className={`${pkg.id % 2 === 0 ? 'lg:order-1' : ''}`}>
                  ...
                  <div className="mb-8">
                    ...
                    >
                      ...
                      {expandedPackage === pkg.id ? <ChevronUp className="w-4 h-4 text-amber-400" /> : <ChevronDown className="w-4 h-4" />}
                    </button>
      ...
      <section className="py-32 bg-zinc-950 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          <h2 className="text-3xl md:text-4xl font-light mb-6">
            {t.tours.customTitle.split(' ')[0]} <span className="font-serif italic text-amber-400">{t.tours.customTitle.split(' ').slice(1).join(' ')}</span>?
          </h2>
